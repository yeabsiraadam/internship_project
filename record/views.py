...
def SignInView(request): 
    ...
    if request.method == "POST":
        ...
        # elif user is None:
            ...
            #  messages.error(request,"You've Been Deactivated. You Cant Login at This Time")
        else:
...
def criminal_list(request):
    crs = Criminal.objects.filter(Status=True,prison=request.user.prison).all().order_by('CriminalName')
    context = {
        'F': crs, 
            }
    formt=SearchForm(request.POST)
    context={'F': crs,'formt':formt,}
    if request.method=='POST':
        criminal=Criminal.objects.all().filter(TimeLeft__contains=formt['TimeLeft'].value())
...
def visitor_list(request):
    visitor = Visitor.objects.filter(prison=request.user.prison).all().order_by('Name')
    context = {
        'visit': visitor
    }
